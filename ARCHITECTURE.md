...
| `estimates`   | Quotes: draft → sent → accepted/declined/expired; convert-to-invoice (Phase 25) — see §4.3                                                                               |
...
| `reminders`   | Daily dunning scan (invoices) + estimate expiry sweep                                                     | ✅ `registerRemindersWorker`, scheduled 08:00 Maldives time (Phase 24, extended Phase 25). Per-business dunning schedule is `businesses.reminderScheduleDays` (day offsets from due date, e.g. `[-3,0,7,14]`); per-invoice opt-out is `invoices.remindersEnabled`. Idempotency: `invoice_reminders_sent`'s unique index on `(invoice_id, offset_days)` guarantees each dunning offset fires at most once even if the scan runs twice. The estimate-expiry half of the same job transitions `sent` estimates past `expiry_date` to `expired` — status transitions are naturally idempotent (already-expired rows are simply skipped), no separate dedup table needed. |
