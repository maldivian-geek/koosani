...
- **`item_name` is free text, not a foreign key into `items`.** A deliberate product decision, same rationale as `expenses.category` (ARCHITECTURE.md §4.11): the owner's spreadsheet never referenced the item master, and forcing every line to resolve to a catalogued item would break the "quick working list" use case (a line can be an item that doesn't exist in the catalogue yet, or never will). Semantically, `item_name` is the **customer's wording** for the item. Every line-returning service response additionally carries a derived, never-stored `systemItemName` — the catalogue item's `name`, resolved at read time by a case-insensitive match of `item_name` against `items.customer_item_name` (via the items **service**, per the cross-module rule; ties on a shared customer name resolve to the alphabetically first item). The detail screen shows it as a read-only "System Item" column.
...
- **Loading columns (owner request, post-1.2.0):** `box_no` (free text, nullable) and `loaded` (boolean, default false) track the physical loading workflow — which box a line was packed into and whether it's on the vehicle. Editable inline on desktop (Box No. as a text cell, Loaded as a checkbox), on the mobile card (checkbox directly; box number via the edit dialog), and in the line dialog; carried on the PDF (Box column + a printed checkbox) and CSV exports. Not part of the paste/image import columns — they're filled in during loading, after the list exists.
...
- **Paste/CSV import is a two-step review-and-confirm flow** (SECURITY.md §13.13): `POST /:id/lines/parse` runs `lib/order-list-import.ts` (papaparse, positional columns matching the owner's sheet, header auto-skip, qty normalization) and persists nothing; the client shows an editable preview, and only the confirmed rows hit `POST /:id/lines/bulk` — one transaction, sequential positions, a single `order_list.import` audit row carrying the row count.
- **Export (Phase 38): PDF and CSV, no numbering surprise since there was never a document number to include.** `GET /order-lists/:id/pdf` renders through the standard `pdf` BullMQ queue (`OrderListDocument.ts`, wired into `worker/pdf.ts` exactly like every other document kind — §8); `GET /order-lists/:id/csv` is streamed directly from the api like `reports.*`'s CSV routes, with no audit row (pure read, same reasoning as `/lines/parse`).
- **Image import (Phase 36) feeds the SAME parse-then-review contract, not a parallel one.** `POST /:id/lines/extract-image` (multipart, one `file` field) exists purely to turn a photo/screenshot of an order table into the identical `{ lines, skipped }` draft shape `/lines/parse` already returns — the client's review screen and the `/lines/bulk` confirm step are unchanged and don't know which path produced the drafts. Pipeline: the uploaded image is **transient** (validated — size cap, magic-byte sniff — then held in memory only, never written to storage/disk, SECURITY.md §13.5 area note) → base64'd into an `extract` BullMQ job → the worker (`worker/extract.ts`, concurrency 1 — OCR is CPU-heavy) runs `lib/ocr-engine.ts`'s `ocrImage()` (tesseract.js, STACK.md) to get word bounding boxes → `lib/order-list-ocr.ts`'s pure `wordsToTsv()` clusters those boxes into table rows/cells (y-center clustering for rows, x-gap clustering for cells, plus a repair pass for OCR's tendency to glue or mis-split the qty cell) → the resulting TSV is handed to the **existing** `lib/order-list-import.ts#parseImportText` — no duplicated header-skip/qty-normalization/Zod-validation logic. The route awaits the job synchronously (`lib/extractClient.ts`, 60s timeout — longer than `pdfClient.ts`'s 20s, since OCR is slower than PDF rendering), same enqueue-and-wait shape as §8's `pdf` queue. No audit row for the extract itself (pure read/transform, same reasoning as `/lines/parse`) — the later `/lines/bulk` confirm still writes the one audit row.
...
| `extract`     | OCR an uploaded order-list image into draft lines (Phase 36 — §4.16)                                      | ✅ `registerExtractWorker`, concurrency 1 (OCR is CPU-heavy). Routes enqueue and synchronously await completion (`lib/extractClient.ts`, 60s timeout), same shape as `pdf`'s enqueue-and-wait. The image itself never touches disk/object storage — only base64 bytes flow through the Redis-backed job payload, discarded once the job finishes.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
