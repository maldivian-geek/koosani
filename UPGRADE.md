...
| F-14 | 🟠  | **Voiding a partially-paid invoice orphans its payments.** `invoicing/service.ts:364` sets `voided` + reversing CN but leaves `payments_received` and `paidAmount` untouched — received money now sits against a voided document with no refund/credit path.                                                                           | Block void while active payments exist (simplest), or auto-reverse/reallocate; decide policy in Phase 20, revisit properly with customer credits in Phase 27. |
| F-15 | 🟡  | **Zero and unbounded overpayments accepted.** Payment `amount` regex admits `0`; `addPayment` never caps against outstanding — `paidAmount` can exceed `total` and status flips to `paid`.                                                                                                                                             | Reject non-positive; cap at outstanding until Phase 27 introduces explicit credit balances.                                                                   |
...
- Credit application model: apply credit notes / overpayments to invoices; customer credit balance; advance/retainer invoices; refunds (recorded, never UPDATE); bad-debt write-off with audit.
- This is the accounting-correct resolution of the void-with-payments and overpayment interim policies from Phase 20.
