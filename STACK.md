...
**Migrations + admin bootstrap on `api` container start.** `api/docker-entrypoint.sh` is the `api` service's `ENTRYPOINT` in `api/Dockerfile.prod`: with no command override, it runs `drizzle-kit migrate` then `api/src/db/seedProd.ts` (compiled to `dist/db/seedProd.js`) before `exec`-ing the actual server — both idempotent, safe on every restart, not just the first. The `worker` service (same image) overrides the container command with `node dist/worker.js`; the entrypoint script detects any passed args (`$# -ne 0`) and just `exec`s them directly, skipping migrate/seed entirely, so only one of the two services ever runs them.
