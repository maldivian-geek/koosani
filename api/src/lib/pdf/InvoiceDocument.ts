...
export type InvoicePdfData = {
  ...
  documentTitle: string // 'TAX INVOICE' or 'CREDIT NOTE'
  number: string
  issueDate: string
  dueDate: string | null
  billTo: PartyInfo
...
export function InvoiceDocument(data: InvoicePdfData): React.ReactElement {
  return h(
    Document,
    null,
    h(
      ...
      h(
        ...
        h(
          ...
          data.business.phone ? h(Text, { style: styles.muted }, data.business.phone) : null,
          data.business.email ? h(Text, { style: styles.muted }, data.business.email) : null,
        ),
        h(
          View,
          { style: styles.titleBlock },
          h(Text, { style: styles.title }, data.documentTitle),
      ...
      h(
        ...
        h(
          ...
          data.billTo.tin ? h(Text, { style: styles.muted }, `TIN: ${data.billTo.tin}`) : null,
        ),
        h(
          View,
          { style: styles.titleBlock },
          h(Text, null, `Issue date: ${data.issueDate}`),
          data.dueDate ? h(Text, null, `Due date: ${data.dueDate}`) : null,
        ),
      ),
      // Line items table
      h(
      ...
      h(
        ...
        data.balanceDue != null
          ? h(
              View,
              { style: styles.totalsRow },
              h(Text, null, 'Balance due'),
              h(Text, null, formatMoney(data.balanceDue)),
            )
          : null,
      ),
      // Notes
      ...
      h(
        ...
        `${data.business.name} — MIRA GST tax invoice`,
      ),
    ),
  )
}
