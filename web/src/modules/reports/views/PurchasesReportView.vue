...
const groupBy = ref<'supplier' | 'item' | 'day'>('supplier')
...
async function run() {
  ...
  try {
    ...
    const data = await apiFetch<ReportResult>(
      `/reports/purchases?from=${from}&to=${to}&groupBy=${groupBy.value}`,
    )
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Could not load purchases report.',
      life: 5000,
    })
  } finally {
    loading.value = false
...
async function exportCsv() {
  ...
  try {
    ...
    await apiFetchDownload(
      `/reports/purchases?from=${from}&to=${to}&groupBy=${groupBy.value}&format=csv`,
      `purchases-${from}-${to}.csv`,
    )
  } catch {
