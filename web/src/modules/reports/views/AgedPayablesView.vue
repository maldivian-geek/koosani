...
async function run() {
  ...
  try {
    const data = await apiFetch<ReportResult>(
      `/reports/aged-payables?asOf=${formatDate(asOf.value)}`,
    )
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Could not load aged payables.',
      life: 5000,
    })
  } finally {
    loading.value = false
...
async function exportCsv() {
  ...
  try {
    ...
    await apiFetchDownload(
      `/reports/aged-payables?asOf=${asOfStr}&format=csv`,
      `aged-payables-${asOfStr}.csv`,
    )
  } catch {
...
<template>
  <div class="space-y-6 pb-12">
    <div class="flex items-center gap-4 flex-wrap">
      ...
      <div class="flex-1">
        <h2 class="text-2xl font-semibold text-surface-900 dark:text-surface-50">Aged Payables</h2>
      </div>
    </div>
    ...
    <div class="card overflow-hidden p-0!">
      <DataTable :value="rows" :loading="loading" :pt="{ root: { class: 'text-sm!' } }">
        <template #empty>
          <div class="text-center py-10 text-sm text-surface-400">No outstanding payables.</div>
        </template>
        <Column field="entityName" header="Supplier" />
        ...
        <Column header="" style="width: 44px">
          <template #body="{ data }">
            <button
              ...
              @click="() => void router.push(`/suppliers/${(data as AgedEntityRow).entityId}/soa`)"
            >
