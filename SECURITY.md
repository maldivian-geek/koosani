...
- Resources: `customers`, `suppliers`, `items`, `inventory`, `invoices`, `bills`, `po`, `gst`, `reports`, `estimates` (Phase 25), `recurring` (Phase 26), `expenses` (Phase 31), `projects` (Phase 32 — covers projects, tasks, and time entries together), `orders` (Phase 34 — order lists; a non-financial checklist resource, same default-policy treatment as every other resource)
...
2. **`export`** (reports bulk CSV) — requires an **explicit grant** in `user_permissions`, even for managers. Also separately rate-limited at 10/hour/user (`rl:reports-bulk-export`), distinct from the general 20/min CSV limiter (§13.6, §13.7).
3. **`view`** (Phase 37 — breaking change from the previous "always allowed" policy) — **manager** gets it by default, same as add/edit/delete below. **staff** gets it only with a grant on that resource: either an explicit `(userId, resource, 'view')` row, or ANY other grant on that resource (`add`/`edit`/`delete`), which implies view — a staff user who can edit invoices can, by construction, also view them. Implemented as `hasAnyGrantOnResource` in `permissions/repository.ts`, a plain exists-check with no `action` filter (an explicit view grant is just a row like any other). `requirePermission(resource, 'view')` is wired onto every GET route (list/detail/pdf/soa/csv-style read) of every permission-gated module — see FUNCTIONS.md's per-module route tables for the full list; the exception is `custom-fields`, whose `GET /custom-fields/values` resolves its resource dynamically per-request from the query's `docType`, same pattern as its `PUT`.
...
Grants are stored in `user_permissions` (`business_id`, `user_id`, `resource`, `action`, `granted_by`, unique on `(user_id, resource, action)`). There is no department-default table — grants are per-user only. Granting/revoking permissions is part of the `users` module (UPGRADE.md Phase 21), including a View column in `UserDrawer.vue`'s permission grid (Phase 37).

**Frontend view gating is cosmetic, not enforcement.** `web/src/shared/ui/SidebarContent.vue` hides a nav item and `web/src/router/index.ts`'s global guard redirects away from a route when the authenticated user lacks `view` on that item's/route's declared `PermissionResource` — both call the same `useAuthStore().hasPermission()` mirror of the policy above. Neither is a security boundary: the api route middleware (`requirePermission(resource, 'view')`) is what actually rejects the request. A user who reaches a screen through some other path (a stale bookmark, a manually-typed URL) still gets a clean 403 from every underlying API call, not a data leak — the frontend gating only avoids landing them on a screen that will visibly fail.
