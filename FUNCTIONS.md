...
| svc          | `auth.resetPassword`           | `(token, newPassword, ctx) → { ok: true; userId } \| { ok: false; reason }`                    | Bumps token_version, deactivates all sessions. Now returns `userId` so `POST /auth/reset-password` can call `invalidateSessionCache(userId)` (SECURITY.md §13.2) — previously `{ ok: true }` only |
...
| route | `PATCH /users/:id`  | `{ name?, role?, permissions? }` → `User & { permissions }`          | `permissions` is a full-replace of the grant set, not a diff. If the body includes `role`, the route calls `invalidateSessionCache(id)` after the service call (SECURITY.md §13.2) |
| route | `DELETE /users/:id` | → `204`                                                              | Soft delete + revoke all sessions; rejects deleting your own account. Route calls `invalidateSessionCache(id)` after the service call                                              |
...
| svc   | `users.update`      | `(businessId, id, UserPatch, ctx) → User & { permissions }`          | Bumps `token_version` (same transaction, via `authRepo.incrementTokenVersion`) when `data.role` is defined and differs from the current role (SECURITY.md §JWT)                    |
| svc   | `users.softDelete`  | `(businessId, id, ctx) → void`                                       | Also bumps `token_version` in the same transaction as the soft delete, for cache-window parity with the session deactivation that follows (SECURITY.md §13.2)                      |
...
| Kind  | Name                          | Signature                                                              | Purpose                                                                                                 |
| ----- | ----------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| route | `GET  /inventory/movements`   | `?itemId&from&to&page` → `MovementRow[]` (joined `itemSku`/`itemName`) | Joined with `items` for display (Phase 33, UPGRADE.md G-13/F-24) — backs the new movement-ledger web UI |
...
| route | `GET    /credit-notes`               | `?customerId&from&to` → `CreditNoteWithCustomer[]`               | Joined with `customers` for `customerName` (Phase 33, UPGRADE.md G-13/F-24 — backs the standalone CN list UI)                                                                                                                                                                                                       |
| route | `GET    /credit-notes/:id`           | → `CreditNote & { lines: CreditNoteLine[] }`                     | Added Phase 33 — previously no single-CN read existed; backs the CN detail view and PDF route                                                                                                                                                                                                                       |
...
| route | `GET    /credit-notes/:id/pdf`       | → `{ url }`                                                      | Renders via the pdf worker queue, same pattern as `/invoices/:id/pdf` (Phase 33 — CN PDF template added, no prior PDF support existed)                                                                                                                                                                              |
| route | `POST   /invoices/:id/delivery-note` | `DeliveryNoteCreate { notes? }` → `DeliveryNote & { lines }`     | Phase 33, UPGRADE.md G-13/F-24 — only from an issued invoice; lines are copied from the invoice's lines with prices dropped                                                                                                                                                                                         |
| route | `GET    /delivery-notes`             | `?customerId` → `DeliveryNoteWithCustomer[]`                     | Phase 33                                                                                                                                                                                                                                                                                                            |
| route | `GET    /delivery-notes/:id`         | → `DeliveryNote & { lines: DeliveryNoteLine[] }`                 | Phase 33                                                                                                                                                                                                                                                                                                            |
| route | `GET    /delivery-notes/:id/pdf`     | → `{ url }`                                                      | Phase 33 — same queue-and-wait pattern as every other document PDF                                                                                                                                                                                                                                                  |
...
| svc   | `invoicing.getCreditNote`            | `(businessId, id) → CreditNote & { lines }`                      | Added Phase 33 — backs `GET /credit-notes/:id` and the PDF renderer                                                                                                                                                                                                                                                 |
| svc   | `invoicing.issueCreditNote`          | `(businessId, creditNoteId, ctx) → CreditNote`                   | Allocates CN number, reverses stock                                                                                                                                                                                                                                                                                 |
| svc   | `invoicing.createDeliveryNote`       | `(businessId, invoiceId, notes, ctx) → DeliveryNote & { lines }` | Phase 33. Throws `ValidationError` unless the invoice is issued; allocates number; copies lines with prices dropped — see ARCHITECTURE.md §4.14                                                                                                                                                                     |
| svc   | `invoicing.getDeliveryNote`          | `(businessId, id) → DeliveryNote & { lines }`                    | Phase 33 — backs `GET /delivery-notes/:id` and the PDF renderer                                                                                                                                                                                                                                                     |
| svc   | `invoicing.listDeliveryNotes`        | `(businessId, { customerId }) → DeliveryNoteWithCustomer[]`      | Phase 33                                                                                                                                                                                                                                                                                                            |
...
`BusinessSettingsPatch` (`shared/src/settings.ts`): `name?, tin?, address?, phone?, email?, allowBackorders?, gstPeriodType?, defaultCreditTermsDays?, defaultInvoiceNotes?, invoiceNumberPrefix?, creditNoteNumberPrefix?, billNumberPrefix?, poNumberPrefix?, estimateNumberPrefix?, defaultEstimateValidityDays?, deliveryNoteNumberPrefix?`.
...
| route | `POST   /order-lists/:id/lines/parse`   | `OrderListParseRequest` → `{ lines: OrderLineCreate[]; skipped }`                      | Parses pasted spreadsheet/CSV text into draft rows; persists nothing (review-and-confirm — SECURITY.md §13.13)         |
| route | `POST   /order-lists/:id/lines/bulk`    | `OrderLinesImport` → `{ lines: OrderListLine[] }` (201)                                | Creates the reviewed batch (max 500) in one transaction; single `order_list.import` audit row with `{ count }`         |
...
| svc   | `orderLists.parseImport`                | `(businessId, orderListId, text) → { lines, skipped }`                                 | Pure transform via `lib/order-list-import.ts` (papaparse, positional columns); asserts the list exists, writes nothing |
| svc   | `orderLists.importLines`                | `(businessId, orderListId, OrderLineCreate[], ctx) → OrderListLine[]`                  | Bulk insert with sequential positions; audited once as `order_list.import`                                             |
...
| route public | `POST /portal/auth/magic-link/verify`              | `{ token }` → `{ id, name, email }` + `portal_session` cookie                                             | Single-use; returns the customer profile directly (from the service's own result — no follow-up `/portal/me` or route-level `customers.assertExists` call needed)                                                                                                                                                                                                                                                                                               |
...
| svc          | `portalAuth.verifyMagicLink`                       | `(token, ctx) → { ok, businessId, customerId, sid, jwt, customer: { id, name, email } } \| { ok: false }` | Atomic single-use consume via `DELETE ... RETURNING`, **then** verifies the customer still exists and isn't soft-deleted (`customers.assertExists(customerId, businessId)` — the customers service, not its repository) **before** creating the session — closes an orphaned-session/500 gap where a customer soft-deleted between token issue and verify previously got a session anyway (SECURITY.md §13.14). Returns `{ ok: false }` if the customer is gone |
...
`portalAuth.repository.createSession` caps active sessions at 10 per `(businessId, customerId)`, evicting the oldest by `lastUsedAt` before insert — mirrors `auth/repository.ts createSession`'s staff session cap (SECURITY.md §Session Management, §13.14).

---

## Module: `portal`

Read-only customer-facing routes (Phase 28, UPGRADE.md G-8) — see ARCHITECTURE.md §4.9. Mounted at `/portal`, gated by `requirePortalAuth` (sets `portalBusinessId`/`portalCustomerId`/`portalSid` on a separate `PortalEnv`, not `AppEnv`). Every route calls straight into the existing `invoicing`/`estimates`/`customers` services, then checks the fetched entity's `customerId` against the session — 404 (never 403) on mismatch. **Draft documents are never shown**: list routes pass `excludeDraft: true` to `invoicing.listInvoices`/`estimates.listEstimates` (new optional param on `ListInvoiceParams`/`ListEstimateParams`, `invoicing`/`estimates` repository+service — staff-facing callers never set it); detail/pdf/accept/decline routes additionally check `entity.status === 'draft'` and return the same `404 not_found` as an ownership mismatch (SECURITY.md §13.14).
